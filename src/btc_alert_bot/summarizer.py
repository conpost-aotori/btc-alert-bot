...
SYSTEM_PROMPT = (
    ...
    "X(旧Twitter)にも全文投稿するため簡潔に。3行ちょうど、"
    "各行32字以内、3行合計95字以内を厳守。"
)
...
5. 数字・固有名詞は厳密に転記、絵文字は1行目冒頭のみ可
6. **X(280字)に全文を載せるため簡潔に**：各行32字以内、3行合計95字以内を厳守
   （冗長な修飾を避け、固有名詞と数字を優先。長い場合は重要度の低い語を削る）
